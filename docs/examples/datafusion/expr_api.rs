...
fn simplify_demo() -> Result<()> {
    ...
    assert_eq!(
        simplifier.simplify(col("i").gt(lit(5)).and(lit(false)).or(col("i").lt(lit(10))))?,
        col("i").lt(lit(10))
    );
...
fn boundary_analysis_and_selectivity_demo() -> Result<()> {
    ...
    let initial_boundaries = vec![ExprBoundaries::try_from_column(&schema, &column_stats, 0)?];
    ...
    let physical_expr = SessionContext::new().create_physical_expr(id_greater_5000, &df_schema)?;
...
fn boundary_analysis_in_conjuctions_demo() -> Result<()> {
    ...
    let physical_expr =
        SessionContext::new().create_physical_expr(age_greater_than_60_less_than_18, &df_schema)?;
...
fn type_coercion_demo() -> Result<()> {
    ...
    let physical_expr = datafusion::physical_expr::create_physical_expr(&expr, &df_schema, &props)?;
    ...
    let physical_expr = SessionContext::new().create_physical_expr(expr.clone(), &df_schema)?;
    ...
    let physical_expr =
        datafusion::physical_expr::create_physical_expr(&coerced_expr, &df_schema, &props)?;
    assert!(physical_expr.evaluate(&batch).is_ok());
