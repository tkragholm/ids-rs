...
async fn to_physical_plan_in_one_api_demo(input: &LogicalPlan, ctx: &SessionContext) -> Result<()> {
...
async fn to_physical_plan_step_by_step_demo(
    ...
    let analyzed_logical_plan =
        ctx.state()
            .analyzer()
            .execute_and_check(input, ctx.state().config_options(), |_, _| ())?;
    ...
    let optimized_logical_plan =
        ctx.state()
            .optimizer()
            .optimize(analyzed_logical_plan, &ctx.state(), |_, _| ())?;
    println!(
