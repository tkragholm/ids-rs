...
use arrow::array::{
    Array, ArrayRef, AsArray, BooleanArray, Int32Array, RecordBatch, StringArray, UInt64Array,
};
use arrow::datatypes::{Int32Type, SchemaRef};
use arrow::util::pretty::pretty_format_batches;
use async_trait::async_trait;
use datafusion::catalog::Session;
use datafusion::common::{internal_datafusion_err, DFSchema, DataFusionError, Result, ScalarValue};
use datafusion::datasource::listing::PartitionedFile;
use datafusion::datasource::memory::DataSourceExec;
use datafusion::datasource::physical_plan::{FileScanConfigBuilder, ParquetSource};
use datafusion::datasource::TableProvider;
use datafusion::execution::object_store::ObjectStoreUrl;
use datafusion::logical_expr::{utils::conjunction, TableProviderFilterPushDown, TableType};
use datafusion::parquet::arrow::arrow_reader::statistics::StatisticsConverter;
use datafusion::parquet::arrow::{arrow_reader::ParquetRecordBatchReaderBuilder, ArrowWriter};
...
impl TableProvider for IndexTableProvider {
    ...
    async fn scan(
        ...
        for (file_name, file_size) in files {
            ...
            file_scan_config_builder = file_scan_config_builder.with_file(PartitionedFile::new(
...
impl ParquetMetadataIndex {
    ...
    pub fn get_files(&self, predicate: Arc<dyn PhysicalExpr>) -> Result<Vec<(&str, u64)>> {
...
impl ParquetMetadataIndexBuilder {
    ...
    fn add_file(&mut self, file: &Path) -> Result<()> {
        ...
        let converter =
            StatisticsConverter::try_new("value", reader.schema(), reader.parquet_schema())?;
        let row_counts = converter
            .row_group_row_counts(row_groups.iter())?
            .ok_or_else(|| internal_datafusion_err!("Row group row counts are missing"))?;
    ...
    fn build(self) -> Result<ParquetMetadataIndex> {
        ...
        let value_column_min: ArrayRef = Arc::new(Int32Array::from(self.value_column_mins));
        let value_column_max: ArrayRef = Arc::new(Int32Array::from(self.value_column_maxs));
...
fn make_demo_file(path: impl AsRef<Path>, value_range: Range<i32>) -> Result<()> {
    ...
    let file_names = StringArray::from_iter_values(std::iter::repeat_n(&filename, num_values));
