...
impl IndexTableProvider {
    ...
    fn create_plan(&self, predicate: &Arc<dyn PhysicalExpr>) -> Result<ParquetAccessPlan> {
        ...
        let pruning_predicate = PruningPredicate::try_new(Arc::clone(predicate), self.schema())?;
...
impl TableProvider for IndexTableProvider {
    ...
    async fn scan(
        ...
        let reader_factory = CachedParquetFileReaderFactory::new(Arc::clone(&self.object_store))
        ...
        let file_scan_config = FileScanConfigBuilder::new(object_store_url, schema, file_source)
            .with_limit(limit)
...
fn make_demo_file(path: impl AsRef<Path>, value_range: Range<i32>) -> Result<()> {
    ...
    let text = StringArray::from_iter_values(value_range.map(|i| format!("TheTextValue{i}")));
