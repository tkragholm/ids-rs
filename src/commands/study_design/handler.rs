...
/// Handle the study design command
pub fn handle_study_design_command(config: &StudyDesignCommandConfig) -> Result<()> {
    ...
    let population_scd_data_path = population_scd_config.output_dir.join("population_scd.parquet");
...
fn perform_matching(
    ...
    info!("Matching {} cases with {} controls (ratio 1:{})", 
          case_pairs.len(), control_pairs.len(), matching_ratio);
    ...
    for (case_idx, (case_pnr, case_birth_date)) in case_pairs.iter().enumerate() {
        ...
        let eligible_control_indices = find_eligible_controls(
            case_pnr,
            *case_birth_date,
            &control_pairs,
            criteria,
        ...
        if eligible_control_indices.is_empty() {
            info!("No eligible controls found for case {} ({}/{})", 
                  case_pnr.value(), case_idx + 1, case_pairs.len());
            continue;
        }
    ...
    if matched_cases.is_empty() {
        return Err(IdsError::Validation("No matches found for any cases".to_string()));
    }
    ...
    info!("Final matched dataset: {} cases and {} controls", 
          matched_cases_batch.num_rows(), matched_controls_batch.num_rows());
