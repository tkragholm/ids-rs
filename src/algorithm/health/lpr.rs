...
use crate::model::icd10::{Icd10Chapter, diagnosis_pattern::normalize_diagnosis_code};
...
pub fn integrate_lpr2_components(
    ...
    let discharge_date_idx = lpr_adm.schema().index_of("D_UDDTO")
    ...
    let _bes_data = if let Some(lpr_bes) = &lpr_bes {
        ...
        let bes_date_idx = lpr_bes.schema().index_of("D_BESDATE")
            .map_err(|e| IdsError::Data(format!("D_BESDATE column not found in LPR_BES: {e}")))?;
    ...
    let diagnoses_by_recnum = map_diagnoses_by_recnum(&diag_recnum_array, &diag_array, &diag_type_array);
    ...
    let mut secondary_diagnoses_list: Vec<Option<Vec<SecondaryDiagnosis>>> = Vec::with_capacity(num_rows);
    let mut diagnosis_chapters = Vec::with_capacity(num_rows);
...
pub fn integrate_lpr3_components(
    ...
    let start_date_idx = lpr3_kontakter.schema().index_of("starttidspunkt")
        .map_err(|e| IdsError::Data(format!("starttidspunkt column not found in LPR3_KONTAKTER: {e}")))?;
    ...
    let end_date_idx = lpr3_kontakter.schema().index_of("sluttidspunkt")
        .map_err(|e| IdsError::Data(format!("sluttidspunkt column not found in LPR3_KONTAKTER: {e}")))?;
    ...
    let diagnoses_by_kontakt_id = map_diagnoses_by_recnum(&diag_kontakt_id_array, &diag_array, &diag_type_array);
...
pub fn combine_harmonized_data(lpr2_data: Option<RecordBatch>, lpr3_data: Option<RecordBatch>) -> Result<RecordBatch> {
...
fn apply_date_filtering(batch: &RecordBatch, config: &LprConfig) -> Result<RecordBatch> {
    ...
    let adm_date_idx = batch.schema().index_of("admission_date")
...
pub fn process_lpr_data(
    ...
    let lpr3_data = if config.include_lpr3 
                      && lpr3_kontakter.is_some()
                      && lpr3_diagnoser.is_some() {
