...
fn main() -> Result<(), Box<dyn Error>> {
    // Generate 1.2M records with 50K treatment cases
    let total_records = 1_200_000;
    let treatment_cases = 50_000;

    println!("Generating test data...");
    let start = std::time::Instant::now();

    generate_pediatric_data("data.csv", total_records, treatment_cases)?;

    println!("Test data generated in {:?}", start.elapsed());
    println!("Total records: {}", total_records);
    println!("Treatment cases: {}", treatment_cases);
    ...
    println!("\nVerification:");
    println!("Actual total records: {}", total_count);
    println!("Actual treatment cases: {}", treatment_count);
