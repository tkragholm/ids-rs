...
pub fn read_parquet(
    ...
    let file = File::open(path).map_err(|e| {
        IdsError::Io(io::Error::new(ErrorKind::NotFound, format!("Failed to open file {}: {}", path.display(), e)))
    })?;

    // Create the reader
    let reader_builder = ParquetRecordBatchReaderBuilder::try_new(file).map_err(|e| {
        IdsError::Data(format!("Failed to read parquet file {}: {}", path.display(), e))
    })?;
    ...
    let reader = if let Some(schema) = schema {
        ...
        reader_builder.with_projection(projection_mask).build().map_err(|e| {
            IdsError::Data(format!("Failed to build parquet reader with projection: {e}"))
        })?
    } else {
        // No projection, read all columns
        reader_builder.build().map_err(|e| {
            IdsError::Data(format!("Failed to build parquet reader: {e}"))
        })?
    };
    ...
    if let Some(pnr_filter) = pnr_filter {
        for batch_result in reader {
            let batch = batch_result.map_err(|e| {
                IdsError::Data(format!("Failed to read record batch: {e}"))
            })?;
...
fn filter_batch_by_pnr(batch: &RecordBatch, pnr_filter: &HashSet<String>) -> Result<RecordBatch> {
    ...
    let pnr_idx = batch.schema().index_of("PNR").map_err(|e| {
        IdsError::Data(format!("PNR column not found in record batch: {e}"))
    })?;
    
    let pnr_array = batch.column(pnr_idx);
    let str_array = pnr_array.as_any().downcast_ref::<StringArray>().ok_or_else(|| {
        IdsError::Data("PNR column is not a string array".to_string())
    })?;
