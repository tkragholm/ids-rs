//! Sampling utilities for record batches
//!
...
/// Write a vector of record batches to a parquet file
pub fn write_parquet(path: &str, batches: &[RecordBatch]) -> Result<()> {
    use parquet::arrow::ArrowWriter;
    use parquet::file::properties::WriterProperties;
    use std::fs::File;

    // If no batches, create an empty file
    if batches.is_empty() {
        return Err(IdsError::Validation("No data to write".to_string()));
    }
    ...
    let file = File::create(path).map_err(IdsError::Io)?;
    ...
    for batch in batches {
        writer
            .write(batch)
            .map_err(|e| IdsError::Data(format!("Error writing batch to parquet: {e}")))?;
    }

    // Close the writer
    writer
        .close()
        .map_err(|e| IdsError::Data(format!("Error closing parquet writer: {e}")))?;
