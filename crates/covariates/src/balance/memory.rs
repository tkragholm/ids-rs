...
impl MemoryManager {
    ...
    pub fn new() -> Self {
        ...
        info!("Memory manager initialized: {:?} tier, {:.2}GB max memory, {} CPUs", 
            tier, max_memory as f64 / (1024.0 * 1024.0 * 1024.0), cpu_count);
    ...
    pub fn get_max_parallel_tasks(&self) -> usize {
        ...
        (self.cpu_count as f64 * parallelism_factor).round() as usize
...
impl Drop for MemoryGuard<'_> {
    fn drop(&mut self) {
        // Safety: We know the pointer is valid because it's from a static OnceLock
        let manager_ptr = self.manager.clone() as usize as *const MemoryManager;
        unsafe {
