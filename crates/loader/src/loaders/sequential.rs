...
impl StoreLoader for SequentialLoader {
    fn load_from_path(base_path: String) -> Result<ArrowStore, IdsError> {
        ...
        // Create an empty store - this is not a Result<ArrowStore> but just an ArrowStore
        ...
        if let Ok(families) = registry::load_family(&base_path, None) {
            store.add_family_data(families);
            progress.inc_main();
        }
        ...
        if let Ok(akm_data) = registry::load_akm(&base_path, None) {
            ...
            store.add_akm_data(current_year, akm_data);
            progress.inc_main();
        }
        ...
        if let Ok(bef_data) = registry::load_bef(&base_path, None) {
            ...
            store.add_bef_data("current".to_string(), bef_data);
            progress.inc_main();
        }
        ...
        if let Ok(ind_data) = registry::load_ind(&base_path, None) {
            ...
            let current_year = chrono::Local::now().year();
            store.add_ind_data(current_year, ind_data);
            progress.inc_main();
        }
        ...
        if let Ok(uddf_data) = registry::load_uddf(&base_path, None) {
            ...
            store.add_uddf_data("current".to_string(), uddf_data);
            progress.inc_main();
        }
    ...
    fn load_with_custom_paths(config: RegisterPathConfig) -> Result<ArrowStore, IdsError> {
        ...
        if let Some(family_path) = paths.get("family") {
            progress.set_main_message("Loading family relations");
            if let Ok(families) = registry::load_family(family_path.to_str().unwrap_or_default(), None) {
                store.add_family_data(families);
                progress.inc_main();
            }
        }
