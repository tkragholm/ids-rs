...
pub fn configure_logging_with_dir(output_dir: &str) -> IdsResult<()> {
    ...
    let log_file = match FileAppender::builder()
        ...
        Err(e) => {
            ...
            return Err(crate::core::IdsError::Io(std::io::Error::new(
                std::io::ErrorKind::Other, format!("Failed to create log file: {e}")
            )));
        }
    };
    ...
    let debug_file = match FileAppender::builder()
        ...
        Err(e) => {
            ...
            return Err(crate::core::IdsError::Io(std::io::Error::new(
                std::io::ErrorKind::Other, format!("Failed to create debug log file: {e}")
            )));
        }
    };
    ...
    let config = match Config::builder()
        ...
        Err(e) => {
            ...
            return Err(crate::core::IdsError::Io(std::io::Error::new(
                std::io::ErrorKind::Other, format!("Failed to build logging config: {e}")
            )));
        }
    };
    ...
    if let Err(e) = log4rs::init_config(config) {
        ...
        return Err(crate::core::IdsError::Io(std::io::Error::new(
            std::io::ErrorKind::Other, format!("Failed to initialize logging: {e}")
        )));
    }
