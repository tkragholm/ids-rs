...
fn handle_balance_check(config: BalanceCheckConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let check_path_exists = |path: &str, path_type: &str| {
        ...
        let path_type_str = if path_obj.is_dir() { "directory" } else if path_obj.is_file() { "file" } else { "path" };
        
        if exists {
            ConsoleOutput::info(&format!("Found {} at {} ({})", path_type, path, path_type_str));
            log::debug!("Found {} at {} ({})", path_type, path, path_type_str);
        } else {
            ConsoleOutput::warning(&format!("{} not found at {} - will attempt to find alternative paths", path_type, path));
    ...
    let normalize_path = |path: &str, register_type: &str| -> String {
        ...
        if path_obj.is_absolute() {
            ...
            if path.contains(base_dir) {
                ...
                log::debug!("Combining base_dir and path: {} + {} -> {}", base_dir, path, full_path);
            ...
            let resolved_path = match path_obj.canonicalize() {
                Ok(canonical) => {
                    log::debug!("Canonicalized path for {}: {} -> {}", register_type, path, canonical.display());
    ...
    let arrow_store_result = if has_custom_paths {
        ...
        let base_dir = match covariate_dir {
            Some(dir) => {
                ...
                if !Path::new(dir).exists() {
                    ConsoleOutput::warning(&format!("Base directory doesn't exist: {}. Will try to use custom paths directly.", dir));
            ...
            None => {
                ...
                ConsoleOutput::info(&format!("No base directory provided, using current directory: {}", current_dir));
        ...
        if let Some(family_path) = custom_paths.get("family") {
            ...
            ConsoleOutput::info(&format!("CRITICAL CHECK: Verifying family file at {}", path_obj.display()));
            if path_obj.exists() {
                if path_obj.is_file() {
                    ...
                    match std::fs::File::open(path_obj) {
                        Ok(mut file) => {
                            ...
                            match std::io::Read::read(&mut file, &mut buffer) {
                                Ok(bytes_read) => {
                                    ConsoleOutput::info(&format!("Successfully read {} bytes from family file", bytes_read));
                                    // Print the first few bytes in hex for debugging
                                    let hex_bytes: Vec<String> = buffer.iter()
                                        .take(bytes_read)
                                        .map(|b| format!("{:02x}", b))
                                        .collect();
                                    ConsoleOutput::info(&format!("First bytes: {}", hex_bytes.join(" ")));
                                }
                                Err(e) => {
                                    ConsoleOutput::error(&format!("Failed to read bytes from family file: {}", e));
                                }
                            }
                        }
                        Err(e) => {
                    ...
                    if let Ok(output) = std::process::Command::new("pqrs")
                        ...
                        if output.status.success() {
                            ...
                            ConsoleOutput::info(&format!("pqrs output: {}", stdout.trim().lines().next().unwrap_or("empty")));
                        } else {
                    ...
                    ConsoleOutput::warning(&format!("Family path exists but is not a file: {}", path_obj.display()));
                }
            } else {
                ConsoleOutput::warning(&format!("Family file not found at: {}", path_obj.display()));
            }
        }
                ...
                if path_obj.is_dir() {
                    ...
                    match std::fs::read_dir(path) {
                        Ok(entries) => {
                            ...
                            ConsoleOutput::info(&format!("Directory {} contains: {}", path, file_list.join(", ")));
                        },
                        Err(e) => {
            ...
            for (reg_type, path) in &debug_custom_paths {
                if reg_type == "family" {
                    ...
                    if file_path.exists() && file_path.is_file() {
                        ConsoleOutput::info(&format!("Family file exists at {}", file_path.display()));
                        
                        // Try to open with standard Rust file API
                        match std::fs::File::open(file_path) {
                            Ok(_) => ConsoleOutput::info("Family file can be opened with Rust std::fs::File"),
                            Err(e) => ConsoleOutput::error(&format!("Failed to open family file with std::fs::File: {}", e)),
                        }
                    }
                } else {
                    ...
                    if dir_path.exists() && dir_path.is_dir() {
                        ...
                        if let Ok(entries) = std::fs::read_dir(dir_path) {
                            ...
                            if let Some(first_file) = parquet_files.first() {
                                ...
                                ConsoleOutput::info(&format!("Checking sample {} file: {}", 
                                                          reg_type, sample_path.display()));
                                                          
                                // Try to open file
                                match std::fs::File::open(&sample_path) {
                                    Ok(_) => ConsoleOutput::info("Sample file can be opened with Rust std::fs::File"),
                                    Err(e) => ConsoleOutput::error(&format!("Failed to open sample file with std::fs::File: {}", e)),
                                }
                            }
                        }
                    }
        ...
        ConsoleOutput::warning("No covariate directory or custom paths specified, using diagnostic mode");
        Err(types::error::IdsError::missing_data("No data source specified"))
    };
    ...
    match balance_result {
        Ok(balance_results) => {
            ...
            if generate_structured_output {
                ...
                let matched_pair_records = match load_matched_pair_records(Path::new(matches_file)) {
                    Ok(records) => records,
                    Err(e) => {
                        ConsoleOutput::warning(&format!("Could not load matched pair records for structured reports: {}", e));
                        Vec::new()
                    }
                };
                ...
                use cli;
                match cli::generate_structured_reports(&balance_results_copy, &matched_pair_records, output_dir) {
                    Ok(_) => {
                        ConsoleOutput::success(&format!("Generated structured reports in {}/report/", output_dir));
                    },
                    Err(e) => {
                        ConsoleOutput::warning(&format!("Failed to generate structured reports: {}", e));
                    }
                }
            }
